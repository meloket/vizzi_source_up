 	...
 	var tzObj = {
		...
		init: function () {
			...
			if (opts.toggleElem) {
				...
				$(opts.toggleElem).click(function (evt) {
					if (shown) {
						shown = false;
					} else {
						opts.debug("SHOWING");
		...
		draw: function () {
			...
			if (opts.toggleElem) {
				// stop bubbling to prevent 
				// dropdown closing out
				tzObj.elem.find(".tz_timezone_container").click(function (evt) {
					evt.preventDefault();
					opts.debug("click tz");
					return false;
				});
				tzObj.regSelect.click(function (evt) {
					evt.preventDefault();
					return false;
				});
			}
			tzObj.regSelect.change(function (evt) {
				evt.preventDefault();
